...
const DEFAULT_PRODUCT_NAMES = [
  ...
  "Vinaigre de Cidre","Vinaigre de Figue de barbarie",
  "Pollen",
  ...
  "Argan gommage",
  "Eau de rose distillé"
];

function getProductNames() {
  try {
    const stored = localStorage.getItem('sn_product_names');
    return stored ? JSON.parse(stored) : [...DEFAULT_PRODUCT_NAMES];
  } catch(e) { return [...DEFAULT_PRODUCT_NAMES]; }
}

function saveProductNames(names) {
  localStorage.setItem('sn_product_names', JSON.stringify(names));
}
...
function openAddProductModal() {
  ...
  // Reset name select
  const nameSel = document.getElementById('product-name-select');
  if (nameSel) nameSel.value = '';
  const customRow = document.getElementById('product-name-custom-row');
...
function setupProducts() {
  ...
  document.getElementById('btn-save-product').addEventListener('click', async () => {
    ...
    const nameSel = document.getElementById('product-name-select');
    const customInp = document.getElementById('product-name-custom-input');
    let name = '';
    if (nameSel && nameSel.value === '__custom__') {
      name = (customInp ? customInp.value : '').trim();
    } else if (nameSel && nameSel.value) {
      name = nameSel.value;
    } else {
      name = (document.getElementById('product-name')||{value:''}).value.trim();
    }
...
function renderProducts() {
  ...
  grid.querySelectorAll('.product-card').forEach(el => {
    el.addEventListener('click', () => {
      ...
      if (unitSel) unitSel.innerHTML = UNITS.map(u => `<option value="${u}">${u}</option>`).join('');
      if (catSel)  catSel.innerHTML  = CATS.map(c  => `<option value="${c}">${c}</option>`).join('');
      // Set name in select or custom
      const nameSel2 = document.getElementById('product-name-select');
      const customRow2 = document.getElementById('product-name-custom-row');
      const customInp2 = document.getElementById('product-name-custom-input');
      populateProductNames();
      if (nameSel2) {
        const found = Array.from(nameSel2.options).some(o => o.value === p.name);
        if (found) {
          nameSel2.value = p.name;
          if (customRow2) customRow2.style.display = 'none';
        } else {
          nameSel2.value = '__custom__';
          if (customRow2) customRow2.style.display = '';
          if (customInp2) customInp2.value = p.name||'';
        }
      }
...
function setupNamesManager() {
  const btnManage = document.getElementById('btn-manage-names');
  if (btnManage) {
  ...
  const btnAdd = document.getElementById('btn-add-name');
  if (btnAdd) {
  ...
  if (input) {
    input.addEventListener('keypress', e => {
      if (e.key === 'Enter') document.getElementById('btn-add-name').click();
    });
  }
}
...
function renderNamesList() {
  ...
  if (!el) return;
  const names = getProductNames();
  if (!names.length) { el.innerHTML = '<div class="empty-state">Aucun nom enregistré</div>'; return; }
  el.innerHTML = names.map((n, i) => `
    <div class="name-item">
      <span>${n}</span>
      <button class="btn-delete-name" data-index="${i}" title="Supprimer">🗑️</button>
    </div>
  `).join('');
  el.querySelectorAll('.btn-delete-name').forEach(btn => {
    btn.addEventListener('click', () => {
      const names = getProductNames();
      const name  = names[parseInt(btn.dataset.index)];
      if (!confirm('Supprimer "' + name + '" de la liste ?')) return;
      names.splice(parseInt(btn.dataset.index), 1);
      saveProductNames(names);
      renderNamesList();
      populateProductNames();
    });
